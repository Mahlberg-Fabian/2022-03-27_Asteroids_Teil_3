...
from pygame.constants import (
    QUIT, KEYDOWN, KEYUP, K_ESCAPE, K_TAB, K_KP_PLUS, K_KP_MINUS, K_F10, K_LEFT, K_RIGHT, K_UP, K_DOWN, K_SPACE, K_1, K_2)
import os
import random
import time
...
class Settings:                                                 # Klasse Settings um Einstellungen zu treffen, sowie z.B. Bilder speicherorte abzurufen
    window_width = 650                                          # Fensterbreite
    window_height = 750                                         # Fensterhöhe
...
class Kaefer(pygame.sprite.Sprite):
    def __init__(self):
        super().__init__()
        self.images = []
        self.images.append(pygame.image.load(os.path.join(Settings.image_path, "käfer.png")).convert_alpha())                # Image laden | convert.alpha() -> Alphakanal soll erhalten bleiben / hintergrund soll unsichtbar gemacht werden
        self.images.append(pygame.image.load(os.path.join(Settings.image_path, "käferblue.png")).convert_alpha())            # Image laden
        self.image = self.images[0]
        self.rect = self.image.get_rect()
        self.rect.bottom = Settings.window_height - 10                                                                       # Der Marienkäfer spawnt -> -10 verschiebt den Käfer etwas nach oben (damit es nicht so aussieht, als würde der Käfer direkt auf der Bildkante aufliegen)
    ...
    def move_left(self):
        self.speed_h = -1 * self.speed            # horizontale Variable wird auf -4 gesetzt (y-Koordinate verringert sich, somit wandert sie nach oben)
    ...
    def rotate_left(self):
        self.image_orig = self.image.rotate_left(self.rect.height - 90, self.rect.width - 90)                            # NEU 

    def rotate_right(self):
        self.image_orig = self.image.rotate_right(self.rect.height + 90, self.rect.width + 90)                           # NEU 
...
class Apfel(pygame.sprite.Sprite):
    ...
    def __init__(self):
        ...
        self.image = pygame.image.load(os.path.join(Settings.image_path, f"apfel{r}.png")).convert_alpha()   # Zufallszahl wird in "{r}" eingesetzt
        self.rect = self.image.get_rect()
...
class Game(object):
    ...
    def __init__(self):
        ...
        self.background = Background("background.jpg")                                 # Hintergrund einfügen
        self.kaefer = Kaefer()
        ...
        for event in pygame.event.get():                               
            if event.type == QUIT:                                  
                ...
                if event.key == K_ESCAPE:                            # Wenn die Escape Taste            wurde
                    ...
                    Kaefer.rotate_left()                 #NEU#

                elif event.key == K_2:
                    Kaefer.rotate_right()               #NEU#
